...
ssize_t razer_attr_write_mode_reactive(IOUSBDeviceInterface **usb_dev, const char *buf, int count)
{
    ...
    UInt16 product = -1;
    (*usb_dev)->GetDeviceProduct(usb_dev, &product);
    debug(GetDeviceProduct : count = %d, count);
    if (count == 4)
    {
...
/**
 * Set Custom Color for a row of keys, row index in 0..5, key index in 0..21
 *
 ...
 */
ssize_t razer_attr_write_matrix_custom_frame(IOUSBDeviceInterface **usb_dev, const unsigned char *buf, int count)
{
    debug("start");
    struct razer_report report;
    int offset = 0;
    ...
    while (offset < count)
    {
        if (offset + 3 > count)
        {
            error("razerkbd: Wrong Amount of data provided: Should be ROW_ID, START_COL, STOP_COL, N_RGB\n");
            break;
        }
        ...
        debug("row : row_id = %d, start_col = %d, stop_col = %d, row_length = %d", row_id, start_col, stop_col, row_length);

        // printk(KERN_ALERT "razerkbd: Row ID: %d, Start: %d, Stop: %d, row length: %d\n", row_id, start_col, stop_col, row_length);

        if (start_col > stop_col)
        {
            error("razerkbd: Start column is greater than end column\n");
            break;
        }

        if (offset + row_length > count)
        {
            error("razerkbd: Not enough RGB to fill row : offset = %d, count = %d", offset, count);
            break;
        }
...
static struct razer_report razer_send_payload(IOUSBDeviceInterface **dev, struct razer_report *request_report)
{
    ...
    if (retval == kIOReturnSuccess)
    {
        ...
        {
            error("Response doesnt match request (keyboard)\n");
        } else if (response_report.status == RAZER_CMD_BUSY) {
            error("Device is busy (keyboard)\n");
        } else if (response_report.status == RAZER_CMD_FAILURE) {
            error("Command failed (keyboard)\n");
        } else if (response_report.status == RAZER_CMD_NOT_SUPPORTED) {
            error("Command not supported (keyboard)\n");
        } else if (response_report.status == RAZER_CMD_TIMEOUT) {
            error("Command timed out (keyboard)\n");
